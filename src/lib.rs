...
impl DateTimeSecond {
    pub fn deserialize<R: Read>(reader: R) -> Result<DateTimeSecond, DeserializationError> {
        let mut bytes = reader.bytes();
        let byte0 = next_byte(&mut bytes)?;
...
impl SubSecond for DateTimeSecond {
    fn fractional_second(&self) -> FractionalSecond {
        self.frac_second
    }
}
